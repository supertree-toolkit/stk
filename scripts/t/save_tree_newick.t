...
ok (Bio::STK::save_tree_newick('t/data/tree3_check.tre', \@trees3), "Saved file 3 OK");
...
ok (Bio::STK::save_tree_newick('t/data/tree1_check.tre', \@tree1, ['tree_test']), "Saved tree with name");
my ($file) = 't/data/tree1_check.tre';
my ($f);
open( $f, "<", $file ) || die " Can't open file $file, quitting.\n";
my $lines = do { local $/; <$f> };
ok($lines =~ m/tree_test/,"Found tree name");
# OK, this test is a bit obvious - course there isn't one!
# But it let's us know if it works if Bio::NEXUS ever let's us save
# a Newick tree
ok($lines !~ m/TAXA/,"Didn't save a taxa block"); 
unlink 't/data/tree1_check.tre';

Bio::STK::save_tree_newick('t/data/tree1_check.tre', \@tree_quoted);
...
eval {
    Bio::STK::save_tree_newick('t/data/thisdirectorydoesnotexist/tree1_check.tre', \@tree_quoted);
};
