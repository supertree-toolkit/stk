...
def generate_species_level_data(XML, taxonomy, ignoreWarnings=False):
        ...
        # work out which level - should we encode this in the data to start with?
            for tl in taxonomy_levels:
                if (t == taxonomy[t][tl]):
                    ...
                    for taxon in taxa:
                        if taxonomy[taxon][current_level] == t and not taxonomy[taxon][SPECIES] == '-': # our current taxon
                            subs.append(taxon)
        # create the sub
        if len(subs) > 0:
            old_taxa.append(t)
            new_taxa.append(subs)
