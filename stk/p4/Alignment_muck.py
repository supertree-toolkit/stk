...
def checkTranslation(self, theProteinAlignment, transl_table=1):
    """Check that self translates to theProteinAlignment.

    Self should be a DNA alignment.  It is translated using
    GeneticCode(transl_table) and compared against
    theProteinAlignment.  The theProteinAlignment sequence order,
    names, and gap pattern should be the same as in the DNA alignment.
    The default transl_table is the standard (or so-called universal)
    genetic code.
    ...
    (These are found in p4.GeneticCode.py)
...
def translate(self, transl_table=1, nnn_is_gap=False):
    """Returns a protein alignment from self, a DNA alignment.

    Self is translated using GeneticCode(transl_table).  At the
    moment, we can only do translations where the frame of the codon
    is 123, ie the first sequence position is the first position of
    the codon.  The default transl_table is the standard (or so-called
    ...
    ``nnn_is_gap`` is for odd alignments where there are long
    ...
    for i in range(len(self.sequences)):
        ...
        for j in range(a.length):
            ...
            if theCodon == '---':
                ...
                protSeq[j] = gc.code[theCodon]
                #print "%s  %s" % (theCodon, protSeq[j])
                #if protSeq[j] == '*':
                #    raise Glitch
            elif theCodon == 'nnn' and nnn_is_gap:
                protSeq[j] = 'x'
