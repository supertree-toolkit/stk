...
def restoreNamesFromRenameForPhylip(self, dictFName='p4_renameForPhylip_dict.py'):
    ...
    if os.path.exists(dictFName):
        import __main__
        exec(compile(open(dictFName, "rb").read(), dictFName, 'exec'), __main__.__dict__,  __main__.__dict__)
...
def restoreDupeTaxa(self, dictFileName='p4DupeSeqRenameDict.py', asMultiNames=True):
    ...
    if not os.path.isfile(dictFileName):
        ...
        raise Glitch(gm)
    loc = {}
    exec(compile(open(dictFileName, "rb").read(), dictFileName, 'exec'), {}, loc)
    try:
...
def dupeSubTree(self, dupeNodeSpecifier, up):
