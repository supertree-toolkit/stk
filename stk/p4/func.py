...
def readFile(fName):
    ...
    if result:
        ...
        if suffix == 'py':
            flob.close()
            import __main__
            exec(compile(open(fName, "rb").read(), fName, 'exec'), __main__.__dict__,  __main__.__dict__)
...
p4 is also the name of a Python script that loads the p4 package.""")
    ...
    if which('info'):
        print("""
If you are using the p4 script interactively (which you probably are
doing right now), you can read about it by typing 'func.info()' at the
p4 prompt.""")
        
    else:
        print("""
There is some documentation, but it needs the gnu 'info' program,
which does not appear to be available on this system.  You may want to
install it.""")
...
def info():
    """Use the 'info' program to display the p4.info documentation."""
    ...
    gm = ["func.info()"]
    if not which('info'):
        gm.append("Can't find the 'info' program, needed to read the documentation.")
        raise Glitch(gm)

    gotDir = False
    try:
        from installation import p4InfoDir
        if os.path.exists(os.path.join(p4InfoDir, 'p4.info')):
            gotDir = True
    except ImportError:
        # No installation.py file-- it was not installed with setup.py
        import p4
        pth = p4.__file__
        #print pth
        ...
        if os.path.exists(pth):
            ...
            if os.path.exists(pth):
                pth = os.path.join(pth, 'info')
                if os.path.exists(os.path.join(pth, 'p4.info')):
                    p4InfoDir = pth
                    gotDir = True
        
    if not gotDir:
        gm.append("Can't find the directory with the p4 info file.")
        raise Glitch(gm)

    theInfoPath = os.getenv("INFOPATH")
    #print theInfoPath
    #print p4InfoDir
    # This next line does not affect the environ hash directly, but it works for os.system() calls.
    os.putenv("INFOPATH", "%s:%s:" % (theInfoPath, p4InfoDir))
    os.system("info p4")
...
def dirichlet1(inSeq, alpha, theMin, theMax, normalizeTo1=True):
    """Return a draw from a Dirichlet distribution with a single alpha value.

    inSeq is a list, and a copy is made, modified, and returned.
    """
    ...
    if normalizeTo1:
        ...
        for i in range(len(theSeq)):
            ...
            theSeq[i] = random.gammavariate(theSeq[i] * alpha, 1.0)
            theSum += theSeq[i]
        for i in range(len(theSeq)):
            theSeq[i] /= theSum
        for i in range(len(theSeq)):
            isGood = False
            #while (theSeq[i] < theMin) or (theSeq[i] > theMax):
            while not isGood:
                if theSeq[i] < theMin:
                    theSeq[i] = (theMin - theSeq[i]) + theMin
                elif theSeq[i] > theMax:
                    theSeq[i] = theMax - (theSeq[i] - theMax)
                else:
                    isGood = True
        theSum = 0.0
        for i in range(len(theSeq)):
        ...
        if 0:
            for i in range(len(theSeq)):
                if theSeq[i] < theMin or theSeq[i] > theMax:
                    if theSeq[i] < theMin:
                        gm.append("theSeq[%i] = %f, less than theMin %f" % (i, theSeq[i], theMin))
                    if theSeq[i] > theMax:
                        gm.append("theSeq[%i] = %f, more than theMax %f" % (i, theSeq[i], theMax))
                    raise Glitch(gm)
        return theSeq
    else:
        for i in range(len(theSeq)):
...
def dirichlet2(inSeq, outSeq, alpha, theMin):
    ...
    inSeq and outSeq are both numpy arrays.  The result is normalized to 1.0
    """
    ...
    for i in range(len(inSeq)):
        #theSeq[i] = pf.gsl_ran_gamma(var.gsl_rng, theSeq[i] * alpha, 1.0)
        outSeq[i] = random.gammavariate(inSeq[i] * alpha, 1.0)
        #if outSeq[i] < theMin:
        #    outSeq[i] = theMin # + (0.1 * theMin * random.random())
    outSeq /= outSeq.sum()
    theMax = 1.0 - theMin
    for i in range(len(outSeq)):
        ...
        while not isGood:
            if outSeq[i] < theMin:
                outSeq[i] = (theMin - outSeq[i]) + theMin
            elif outSeq[i] > theMax:
                outSeq[i] = theMax - (outSeq[i] - theMax)
            else:
                isGood = True
    outSeq /= outSeq.sum()
    # check
    if 0:
        for i in range(len(inSeq)):
            if outSeq[i] < theMin or outSeq[i] > theMax:
                if outSeq[i] < theMin:
                    gm.append("theSeq[%i] = %f, less than theMin %f" % (i, outSeq[i], theMin))
                if outSeq[i] > theMax:
                    gm.append("theSeq[%i] = %f, more than theMax %f" % (i, outSeq[i], theMax))
                raise Glitch(gm)
...
def recipes(writeToFile=var.recipesWriteToFile):
    ...
    You can see these in the info file, also.
    """
...
if var.usePfAndNumpy:
    ...
    def gsl_ran_dirichlet(alpha, theta, result, work, seed=None):
        """Make a random draw from a dirichlet distribution.

        alpha, theta, result, and work should be NumPy arrays
        The draw ends up in result.
        ...
        try:
            ...
            assert type(work) == numpy.ndarray
        except AssertionError:
            gm.append(" alpha, theta, result, and work should be all be numpy arrays.")
            raise Glitch(gm)

        assert len(alpha) > 1
        assert len(theta) == len(alpha)
        assert len(result) == len(alpha)
        assert len(work) == len(alpha)
        ...
        pf.gsl_ran_dirichlet(var.gsl_rng, len(theta), alpha, theta, result, work)
    ...
    def summarizeMcmcPrams(skip=0, run=-1, theDir='.'):
        ...
        try:
            loc = {}
            exec(compile(open(os.path.join(theDir, "mcmc_pramsProfile.py"), "rb").read(), os.path.join(theDir, "mcmc_pramsProfile.py"), 'exec'), {}, loc)
